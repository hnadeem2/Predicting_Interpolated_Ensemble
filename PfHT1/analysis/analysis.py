...
gen_pdbs_A =  natsort.natsorted(glob.glob("aggregated_pdbs/*_A_*.pdb"))
gen_pdbs_B =  natsort.natsorted(glob.glob("aggregated_pdbs/*_B_*.pdb"))
...
if_model_x, if_model_y = return_dis(["IF_model_0.pdb"])
of_model_x, of_model_y = return_dis(["OF_model_0.pdb"])
gen_x_A, gen_y_A = return_dis(gen_pdbs_A)
gen_x_B, gen_y_B = return_dis(gen_pdbs_B)
...
for x,y,text in zip(gen_x_A,gen_y_A,gen_pdbs_A):
	round_number = re.search(r'round(\d+)', text).group(1)
	plt.annotate(round_number,xy=(x,y),fontsize=4)
	plt.scatter(x,y,marker='o', color = 'yellow')

for x,y,text in zip(gen_x_B,gen_y_B,gen_pdbs_B):
	round_number = re.search(r'round(\d+)', text).group(1)
	plt.annotate(round_number,xy=(x,y),fontsize=4)
