...
def run_Boltz(input_path,
    ...
    shared_cache = f"Boltz_output_{lambda_param}/cache_{direction}"
...
def mix_prob(path_to_prob1, path_to_prob2, lambda_param, round_no, label="A"):
    ...
    # Save FASTA
    fasta_dir = f'fasta_Boltz_input_{lambda_param}/round{round_no}_{label}'
...
def main(rounds, lambda_param, s1_pdb, s2_pdb):
    ...
    for direction in ["A", "B"]:  # A: s1 -> new, B: s2 -> new
        ...
        _, anchor_prob = run_ProteinMPNN(pdb_path=anchor_pdb, output_dir=f'pMPNN_output_{lambda_param}/{direction}_anchor')
        _, changing_prob = run_ProteinMPNN(pdb_path=changing_pdb, output_dir=f'pMPNN_output_{lambda_param}/{direction}_changing')

        for round_num in range(1, ROUNDS + 1):
            print(f"\n=== Round {round_num} ({direction}) ===")

            round_output_dir = f'Boltz_output_{lambda_param}/round{round_num}_{direction}'
            ...
            latest_output_dir = f'pMPNN_output_{lambda_param}/round{round_num}_{direction}_mpnn'
...
if __name__ == '__main__':
    ...
    for l in args.lambda_param_list:
        main(args.rounds, l, args.s1_pdb, args.s2_pdb)
